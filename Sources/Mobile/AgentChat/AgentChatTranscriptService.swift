...
final class AgentChatTranscriptService {
    ...
    /// that had no session yet, keyed by agent + surface id. Bounds the
    /// main-actor directory walk to once per `detectionScanThrottle` while a
    /// detected agent has not yet written its transcript; a successful adoption
    /// removes the entry.
    ...
    private func observeAgentTitleChanges(adoptDetectedAgentSessions: @escaping @MainActor (String) -> Void) {
        ghosttyTitleSubscription = GhosttyTitleChangeSubscription { change in
            let title = change.title.lowercased()
            guard title.contains("claude")
                || title.contains("codex")
                || title.contains("antigravity")
                || Self.textContainsAgentToken(title, token: "agy")
                || change.title.trimmingCharacters(in: .whitespacesAndNewlines).hasPrefix("✳") else {
                return
            }
    ...
    func adoptDetectedClaudeSession(
        ...
        if let bound = registry.sessions(workspaceID: nil)
            ...
            if let resolved = newestClaudeTranscript(
                ...
                detectionScanAt.removeValue(forKey: detectionScanKey(agentKind: .claude, surfaceID: surfaceID))
                registry.update(sessionID: bound.sessionID) { record in
