...
extension SessionRemoteWorkspaceSnapshot {
    func workspaceConfiguration(
        ...
        let defaultFreestyleVMID = Self.defaultFreestyleVMID(
            ...
            persistentDaemonSlot: normalizedPersistentDaemonSlot,
        ...
        return WorkspaceRemoteConfiguration(
            ...
            persistentDaemonSlot: (preservePTYSession || restoreDefaultFreestyleSSHD) ? normalizedPersistentDaemonSlot : nil,
            skipDaemonBootstrap: skipDaemonBootstrap == true
        )
    }
    ...
    private static func defaultFreestyleVMID(
        destination normalizedDestination: String,
        persistentDaemonSlot normalizedPersistentDaemonSlot: String?,
        skipDaemonBootstrap: Bool
    ) -> String? {
        guard skipDaemonBootstrap,
              normalizedPersistentDaemonSlot == "cmux-default-freestyle-sshd-v1",
    ...
    private static func defaultFreestyleSSHAttachCommand(vmID: String) -> String {
        let lines = [
            ...
            "exec \"$cmux_freestyle_cli\" vm ssh-attach --id \(shellQuote(vmID)) --default-freestyle-sshd",
        ]
