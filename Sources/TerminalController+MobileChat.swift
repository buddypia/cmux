...
extension TerminalController {
    ...
    private func adoptDetectedAgentSession(
        ...
        service: AgentChatTranscriptService? = nil
    ) -> Bool {
        guard let service = service ?? agentChatTranscriptService else { return false }
        ...
        let resolvedWorkspaceID = workspaceID ?? workspace.id.uuidString
        switch agentKind {
        case .claude:
        ...
        case .antigravity:
            return service.adoptDetectedAntigravitySession(
                workspaceID: resolvedWorkspaceID,
                surfaceID: panel.id.uuidString,
                workingDirectory: cwd
            )
        case .codex:
            return service.adoptDetectedCodexSession(
                workspaceID: resolvedWorkspaceID,
                surfaceID: panel.id.uuidString,
                workingDirectory: cwd
            )
