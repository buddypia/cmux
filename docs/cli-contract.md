...
| `hooks <agent> <event>` | Generic hook surface for `grok`, `opencode`, `pi`, `amp`, `cursor`, `gemini`, `kiro`, `antigravity` (or `agy`), `rovodev`, `hermes-agent`, `copilot`, `codebuddy`, `factory`, and `qoder`. |
